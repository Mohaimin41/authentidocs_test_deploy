...
export async function POST({
  ...
  if (filename === null) {
    console.error(
      "ERROR @api/team/addteamchunkfile/continue:19: url parameter filename returned null"
    );
    get(filemap).clear();
  ...
  if (
    ...
    console.error(
      "ERROR @api/team/addteamchunkfile/continue:33: request json null||undefined or has incorrect body"
    );
    get(filemap).clear();
    return json({ success: false });
  }
