...
export async function POST({
  ...
  if (
    ...
    console.log(
      "ERROR @api/notice/addnoticechunkfile/finish:22: filename or given_noticeid returned empty/undefined from url"
    );
    get(filemap).clear();
  ...
  if (file_array === undefined) {
    console.log(
      "ERROR @api/notice/addnoticechunkfile/finish:32: array filearray returned as undefined from filemap"
    );
    get(filemap).clear();
    return json({ success: false });
  }
