...
export async function POST({
  ...
  if (
    ...
    console.error(
      "ERROR @api/org/addorgchunkfile/finish:27: filename or given_orgid returned empty/undefined from url"
    );
    get(filemap).clear();
  ...
  if (file_array === undefined) {
    console.error(
      "ERROR @api/org/addorgchunkfile/finish:37: array filearray returned as undefined from filemap"
    );
    get(filemap).clear();
    return json({ success: false });
  }
