...
export async function POST({
  ...
  if (!session?.user) {
    return new (error as any)(401, "You must be logged in to get forum top level post data");
  }
  ...
  if (given_forumid === undefined || given_forumid === null) {
    console.error(
      "ERROR @api/forum/getforumtoplevelpost:21: invalid user input error:\n",
      forum_info
    );
  ...
  if (_error) {
    console.error(
      "ERROR @api/forum/getforumtoplevelpost:40: supabase getting top level post data error\n",
      _error
    );
    return new (error as any)(
      500,
