<script lang="ts">
  ...
  import { make_date, make_pfp_url } from "$lib/helpers";
  ...
  async function make_moderator(): Promise<void> {
    let response: Response = await fetch("/api/org/makeadmin", {
      method: "POST",
      headers: {
  ...
  onMount((): void => {
    ...
    pubkey = [
      ...new Uint8Array(new TextEncoder().encode(JSON.stringify(pub_key))),
    ]
...
<button
  data-modal-target="member-modal-{id}"
  data-modal-toggle="member-modal-{id}"
  class="block text-start flex space-x-2 p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700"
  style="width: 100%;"
>
  <img
      ...
      <div class="p-4 md:p-5 space-y-4">
        <div class="flex justify-end">
          <button
            ...
            data-modal-hide="member-modal-{id}"
          >
            <svg
              class="w-3 h-3"
              aria-hidden="true"
